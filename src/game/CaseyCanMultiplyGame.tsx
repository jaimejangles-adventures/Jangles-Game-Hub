...
const COUNT_WORDS = ['', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten', 'eleven', 'twelve', 'thirteen', 'fourteen', 'fifteen', 'sixteen'];
...
const MSGS = {
  intro:   ["Let's multiply! ✖️", "Count the groups!", "How many in all?", "Times tables time!"],
...
function generateRound(excludeIdx?: number) {
  ...
  while (wrongs.size < 2) {
    const w = rand(Math.max(1, answer - 4), Math.min(16, answer + 4));
...
export function CaseyCanMultiplyGame({ onComplete }: { onComplete?: () => void } = {}) {
      ...
      <div className="absolute inset-0 z-10 flex items-center justify-center pointer-events-none">
        ...
        >
          ...
          <div className="flex flex-col items-center gap-1">
            <span className="text-base font-extrabold tracking-tight" style={{ color: '#1a1a2e' }}>Casey Can Multiply! ✖️</span>
          ...
          <div className="flex items-center justify-center gap-3 flex-wrap">
            <ObjectGroup key={`a-${round.rdIdx}`} count={a} obj={obj} color="#fde68a" muted={muted} onAllTapped={() => setAAllTapped(true)} />

            <motion.button
              whileTap={aAllTapped ? { scale: 0.85 } : {}}
              onClick={() => aAllTapped && speak('times', muted)}
            ...
            <ObjectGroup key={`b-${round.rdIdx}`} count={b} obj={obj} color="#c4b5fd" muted={muted} />

            <motion.button
              whileTap={aAllTapped ? { scale: 0.85 } : {}}
              onClick={() => aAllTapped && speak('equals', muted)}
              className="flex items-center justify-center w-10 h-10 rounded-full border-[3px] border-ink bg-white font-extrabold text-2xl shadow"
              style={{ borderBottomWidth: 4, borderRightWidth: 3, opacity: aAllTapped ? 1 : 0.35, cursor: aAllTapped ? 'pointer' : 'default' }}>
              =
            </motion.button>

...
const MSGS = {
  intro:   ["Let's divide! ➗", "Split them up!", "How many in each group?", "Share them equally!"],
...
function generateRound(excludeIdx?: number) {
  // quotient × divisor = dividend; answer is the quotient
  const quotient = rand(1, 3);
  ...
  while (wrongs.size < 2) {
    const w = rand(Math.max(1, quotient - 3), Math.min(8, quotient + 3));
    if (w !== quotient) wrongs.add(w);
  }

  const choices = [quotient, ...wrongs].sort(() => Math.random() - 0.5);
  return { dividend, divisor, answer: quotient, obj: rd.obj, bg: rd.bg, country: rd.country, music: rd.music, rdIdx, choices };
}

type Phase = 'playing' | 'correct' | 'wrong';

export function CaseyCanDivideGame({ onComplete }: { onComplete?: () => void } = {}) {
  ...
  const { dividend, divisor, obj, bg, country, choices } = round;
      ...
      <div className="absolute inset-0 z-10 flex items-center justify-center pointer-events-none">
        ...
        >
          ...
          <div className="flex flex-col items-center gap-1">
            <span className="text-base font-extrabold tracking-tight" style={{ color: '#1a1a2e' }}>Casey Can Divide! ➗</span>
          ...
          {/* Equation row: [dividend objects] ÷ [divisor objects] = ? */}
          <div className="flex items-center justify-center gap-3 flex-wrap">
            <ObjectGroup key={`div-${round.rdIdx}`} count={dividend} obj={obj} color="#fde68a" muted={muted} onAllTapped={() => setAllTapped(true)} />

            <motion.button
              whileTap={allTapped ? { scale: 0.85 } : {}}
              onClick={() => allTapped && speak('divided by', muted)}
              ...
              style={{ borderBottomWidth: 4, borderRightWidth: 3, opacity: allTapped ? 1 : 0.35, cursor: allTapped ? 'pointer' : 'default' }}>
              ÷
            </motion.button>

            <ObjectGroup key={`dvr-${round.rdIdx}`} count={divisor} obj={obj} color="#67e8f9" muted={muted} />

            <motion.button
              whileTap={allTapped ? { scale: 0.85 } : {}}
              onClick={() => allTapped && speak('equals', muted)}
              className="flex items-center justify-center w-10 h-10 rounded-full border-[3px] border-ink bg-white font-extrabold text-2xl shadow"
              style={{ borderBottomWidth: 4, borderRightWidth: 3, opacity: allTapped ? 1 : 0.35, cursor: allTapped ? 'pointer' : 'default' }}>
              =
            </motion.button>
...
function ObjectGroup({ count, obj, color, muted, onAllTapped }: { count: number; obj: { src: string; label: string }; color: string; muted: boolean; onAllTapped?: () => void }) {
  const [items, setItems] = useState<Array<{ tapped: boolean; num: number | null }>>(() =>
    Array.from({ length: count }, () => ({ tapped: false, num: null }))
  );
  const tappedSoFar = items.filter(it => it.tapped).length;
  ...
  return (
    <div
      ...
      style={{
        ...
        borderBottomWidth: 4,
        borderRightWidth: 3,
        maxWidth: count <= 2 ? 100 : count <= 4 ? 120 : 160,
      }}
    >
      {items.map((item, i) => (
        <motion.button
          ...
          whileTap={{ scale: 0.82 }}
          className={cn(
            'relative w-10 h-10 rounded-xl p-1 border-2 flex items-center justify-center cursor-pointer',
            item.tapped ? 'bg-white border-ink/40' : 'bg-white/80 border-ink/20',
          )}
        >

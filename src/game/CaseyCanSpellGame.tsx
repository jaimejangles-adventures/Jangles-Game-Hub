...
export function CaseyCanSpellGame({ onComplete }: { onComplete?: () => void } = {}) {
  ...
  // Keyboard input — letter keys spell; Enter advances on correct
  useEffect(() => {
    ...
    <div className="relative flex h-full overflow-hidden select-none">
      ...
      <div className="relative z-10 flex h-full w-full items-center justify-center px-4">
        ...
        >
          ...
          {/* Word image — tap to hear the word */}
          ...
          <div className="flex flex-col items-center gap-1.5">
            {KEYBOARD_ROWS.map((row, rowIdx) => (
              <div key={rowIdx} className="flex gap-1.5">
                {row.map((char) => {
                  const isShaking = shakingKey === char;
                  const isFlashing = flashKey === char;
                  return (
                    <motion.button
                      key={char}
                      ...
                      whileTap={phase === 'spelling' ? { scale: 0.82 } : undefined}
                      className="flex items-center justify-center rounded-lg border-[2.5px] border-ink text-sm font-extrabold shadow-sm"
                      style={{
                        width: 36, height: 40,
                        background: isFlashing ? '#86efac' : isShaking ? '#fca5a5' : '#f8fafc',
                        borderBottomWidth: 4,
                        borderRightWidth: 3,
                        color: '#1a1a2e',
                        transition: 'background 0.15s',
                        opacity: phase === 'correct' ? 0.4 : 1,
                      }}
                    >
                      {char.toUpperCase()}
                    </motion.button>

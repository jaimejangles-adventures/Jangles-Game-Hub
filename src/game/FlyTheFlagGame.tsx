...
export function FlyTheFlagGame({ onComplete }: { onComplete?: () => void } = {}) {
  ...
  const { saveScore, saving, saved, reset: resetScore } = useScore("fly-the-flag");
    ...
    return (
      ...
      >
        ...
        <div className="rounded-[2rem] border-[3px] border-ink p-4" style={{ background: "#1a1a2e", borderBottomWidth: 6, borderRightWidth: 5 }}>
          <div className="text-[0.6rem] font-extrabold uppercase tracking-[0.2em] text-gray-500 mb-1">🏆 Top Scores — Fly the Flag</div>
          {user ? (
          ...
          <Leaderboard gameSlug="fly-the-flag" limit={5} theme="dark" />
        </div>

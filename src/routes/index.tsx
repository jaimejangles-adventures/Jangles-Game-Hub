...
function ScrollRow({ games }: { games: typeof GAME_MANIFEST }) {
  ...
  return (
    <div className="relative">
      ...
      >
        {games.map((game) => (
          ...
          >
            {/* Image banner */}
            {(() => {
              const isScene = game.imageVariant === "scene" || game.slug === "fly-the-flag";
              return (
                <div
                  ...
                  style={{
                    ...
                    height: "6rem",
                    padding: isScene ? 0 : "0 0.75rem",
                  }}
                >
                  <img
                    src={game.image}
                    alt={game.title}
                    className={isScene ? "w-full h-full" : "h-20 w-auto object-contain"}
                    style={{
                      maxWidth: "100%",
                      mixBlendMode: isScene ? "normal" : "multiply",
                      objectFit: isScene ? "cover" : undefined,
                      objectPosition: isScene ? "50% 20%" : undefined,
                      maskImage: isScene
                        ? "linear-gradient(to bottom, black 70%, transparent 100%)"
                        : game.slug === "world-adventure"
                        ? "linear-gradient(to right, transparent 0%, black 22%)"
                        : undefined,
                      WebkitMaskImage: isScene
                        ? "linear-gradient(to bottom, black 70%, transparent 100%)"
                        : game.slug === "world-adventure"
                        ? "linear-gradient(to right, transparent 0%, black 22%)"
                        : undefined,
      ...
      {/* Left fade + arrow */}
      {canScrollLeft && (
        <div className="pointer-events-none absolute left-0 top-0 bottom-3 w-16 z-10"
          style={{ background: "linear-gradient(to right, #FFFBF0 30%, transparent)" }}
        >
          <button
            className="pointer-events-auto absolute left-1 top-1/2 -translate-y-1/2 flex h-9 w-9 items-center justify-center rounded-full border-[2.5px] border-ink bg-white text-lg font-extrabold shadow-sm transition-transform hover:scale-110 active:scale-95"
      ...
      {/* Right fade + arrow */}
      {canScrollRight && (
        <div className="pointer-events-none absolute right-0 top-0 bottom-3 w-20 z-10"
          style={{ background: "linear-gradient(to left, #FFFBF0 30%, transparent)" }}
        >
          <button
            className="pointer-events-auto absolute right-1 top-1/2 -translate-y-1/2 flex h-9 w-9 items-center justify-center rounded-full border-[2.5px] border-ink bg-white text-lg font-extrabold shadow-sm transition-transform hover:scale-110 active:scale-95"
            style={{ borderBottomWidth: 4, borderRightWidth: 3 }}
